...
int main(void){
    ...
    /* Binary inference */
    float test[IMG_SIZE]; sample_poly_image(polys[1],0.0f,test);
    int outWb=IMG_W-K+1,outHb=IMG_H-K+1;
    float *featb=malloc(CONV_F*outWb*outHb*sizeof(float));
    bconv_layer(test,Wbconv,Bbconv,IMG_W,IMG_H,K,CONV_F,featb);
    float *hidb=malloc(HIDDEN_N*sizeof(float));
    blinear_layer(featb,Wbhid,Bbhid,CONV_F*outWb*outHb,HIDDEN_N,hidb);
    float outb[NUM_CLASSES];
    ...
    printf("Binary inference predicted class %d\n",pred);

    /* Visualization */
    for(int f=0;f<CONV_F;f++){
        float filt[K2];for(int i=0;i<K2;i++)filt[i]=(Wbconv[f*K2+i]+1.0f)/2.0f;
        char fn[64];sprintf(fn,"filter_bin_%d.dat",f);write_matrix(fn,filt,K,K);
        char outp[64];sprintf(outp,"filter_bin_%d.png",f);
        ...
        sprintf(fn,"feat_bin_%d.dat",f);write_matrix(fn,fmap,outHb,outWb);
        sprintf(outp,"feat_bin_%d.png",f);gplot_png(fn,outp,"Binary feature map");
    }
